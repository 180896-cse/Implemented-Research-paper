...
"""
for i in range(grandsize/10):
	SP.append(CSP("SP "+str(i), [0.155*random.random(), 0.309*random.random(), 0.618*random.random()], [0.155, 0.309, 0.618], 32, 120, 640, 1, 1, 0.39+0.1*random.random(), 1, 0.39+0.1*random.random()))
	
for i in range(grandsize/10, grandsize):
	SP.append(CSP("SP "+str(i), [0.155*random.random(), 0.309*random.random(), 0.618*random.random()], [0.155, 0.309, 0.618], 32, 120, 640, 1, 1, 0.89+0.1*random.random(), 1, 0.89+0.1*random.random()))
...
def getRCost(fd, j):
	...
	for i in range(1, N):
		a = 1
		for k in range(i):
			a = a*(1-temp[k].getQra())*temp[i].getQra()*temp[i].cost[j]
		s = s+a
	return s
...
def getSPsat(spi):
	return 0.5*sat(v([spi]), sum(spi.getCost()), 0.01*sum(spi.getCost())) + 0.5*sat(spi.getQra(), 1, 0.2)
...
def getFsat(fd):
	if len(fd) == 0:
		return 0
	return 0.5*sat(v(fd), sum([sum(spi.getCost()) for spi in fd])/len(fd), 0.01* sum([sum(spi.getCost()) for spi in fd])/len(fd)) + 0.5*sat(getQra(fd), 1, 0.2)
