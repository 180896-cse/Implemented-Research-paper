...
#FS = [x for x in powerset(GrandFederation)]
...
for i in range(1):
	...
	#print "Federation Structure after merging:"
	#for fed in FS:
...
#fsize = len(finalfed)
#fsat = getFsat(finalfed, request)
#fq = getQ(finalfed)
#fa = getQra(finalfed)
#fprofit = totalprofit
#fspprofit = fspprofit / fsize
#fspsat = fspsat / fsize
#print "Lena (43%% Untrusted)\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f" %(fsat, fspsat, fq, fa, fprofit, fspprofit, fsize, runtime)
