...
def getRProfit(fd, j):
	return  (4)*(getRPrice(fd, j) - getRCost(fd, j))
...
def mergepass(P):
	...
	for fd in P:
		...
		j=i+1
		lengthTracker = len(P)
		while j<lengthTracker:
			if getQ(fd+P[j])>max(getQ(fd),getQ(P[j])):
				fd.extend(P[j])
				P.remove(P[j])
...
def splitpass(P):
	changed = False
	for fd in P:
		for subset in powerset(fd):
			if len(subset) < 1 or len(subset) == len(fd):
				continue
			leftover = [x for x in fd if x not in subset]
			if min(getQ(subset),getQ(leftover))>getQ(fd):
				P.remove(fd)
				P.append(subset)
				P.append(leftover)
				changed = True
...
for fd in partition:
	#print "\n"
	fsize = fsize + len(fd)
	fsat = fsat + getFsat(fd)
	fq = fq + getQ(fd)
	fa = fa + getQra(fd)
	fprofit = fprofit + v(fd)
	for sp in fd:
		count = count+1
		fspsat = fspsat + getFSPsat(SP, fd, sp)
		fspprofit = fspprofit + getShapleyPayoff(SP, sp)
		
		#print "%s\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%d\t%f" % (sp.getName(), getShapleyPayoff(SP, sp), getFSPsat(SP, fd, sp), getSPsat(sp), getFsat(fd), v(fd), v([sp]), getQra(fd), getQ(fd), sp.getQra(), len(fd), runtime) 
		

fsat = fsat / len(partition)
fspsat = fspsat / count
fq = fq / len(partition)
fa = fa / len(partition)
fprofit = fprofit / len(partition) 
fspprofit = fspprofit / count
fsize = fsize / len(partition)

print "Khosrowshahi-Krzysztof (43%% Untrusted)\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f" %(fsat, fspsat, fq, fa, fprofit, fspprofit, fsize, runtime)
